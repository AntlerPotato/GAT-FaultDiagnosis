...
from run_single_sample_vis import extract_features
...
def get_fixed_hidden_sizes(output_size: int):
    h1 = min(output_size * 4, 2048)
    h2 = min(output_size * 2, 1024)
    return [h1, h2]
...
def main():
    ...
    logger.info(f"=== Batch Comparison Fixed Hidden (Dim={dimension}, Fault={fault_rate}) ===")
    ...
    hidden_sizes = get_fixed_hidden_sizes(n_nodes)

    logger.info("Training Ordinary GNN with fixed hidden sizes...")
    model_ord = GNN(
        input_size=topo.syndrome_size,
        output_size=n_nodes,
    ...
    logger.info("Training Optimized GNN with fixed hidden sizes...")
    model_opt = GNN(
        input_size=input_size_opt,
        output_size=n_nodes,
        hidden_sizes=hidden_sizes,
    )
    ...
    logger.info("=== Batch Results Fixed Hidden (50 test samples expected) ===")
    logger.info(
    ...
    report_name = f"gnn_batch_compare_fixed_{dimension}d_{fault_rate}.txt"
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(
            f"=== GNN Batch Comparison Fixed Hidden (Dim={dimension}, Fault={fault_rate}) ===\n\n"
        )
        f.write(
    ...
    logger.info(f"Batch comparison report (fixed hidden) saved to {report_path}")
