...
Input:
  ...
  - 本地: .base.BaseModel, utils.logger.get_logger
Output:
  - BPNN: 反向传播神经网络模型，用于从 syndrome 预测故障节点
Position: 实现具体的 BPNN 模型，继承自 BaseModel，提供训练和预测功能
...
class BPNN(BaseModel):
    ...
    def __init__(self, input_size: int, output_size: int, hidden_sizes: list = None):
        ...
        Args:
            ...
            hidden_sizes: 隐藏层大小列表，默认 [节点数*8, 节点数*4, 节点数*2]
        """
        # 默认隐藏层大小 - 增加网络容量
        if hidden_sizes is None:
            hidden_sizes = [output_size * 8, output_size * 4, output_size * 2]
        ...
        for i, h_size in enumerate(hidden_sizes):
            # 每个隐藏层包含: 全连接 → 激活 → BatchNorm → Dropout
            layers.append(nn.Linear(prev_size, h_size))  # 全连接层
            layers.append(nn.ReLU())                      # 激活函数
            layers.append(nn.BatchNorm1d(h_size))         # 批归一化，加速收敛
            # 最后一层不使用Dropout
            if i < len(hidden_sizes) - 1:
                layers.append(nn.Dropout(0.1))           # 降低Dropout率到10%
            prev_size = h_size
        ...
        self.optimizer = optim.Adam(self.network.parameters(), lr=0.0005, weight_decay=1e-5)
