...
> **注意**：当前代码库克隆自指导老师提供的 GitHub 项目并做了改进。BPNN 基线诊断准确率已达 99%+，GAT 模型已实现并通过 4D~8D 全维度验证（4D 98.6%，8D 92.8%，均大幅领先 BPNN）。
...
采用"每次移除一个"策略（完整模型 + 3 个变体 = 4 次实验）：

1. 多头注意力（8头）→ 单头注意力（1头）
2. 双向特征（$2d$）→ 单向特征（$d$）
3. 2 层 GAT → 1 层 GAT

> 正则化消融（去掉 BatchNorm + Dropout）不纳入本文实验。超立方体为正则图，所有节点度数相同，正则化策略在此类均匀结构上的效果有待进一步研究，已列入未来工作。
